...
public class BaseIT {
    ...
    protected static String certificate = "A61_Supplier";

    protected static String keyName = "A61_Mediator";
	...
	public static void oneTimeSetup() throws Exception {
		...
		int timeoutConnect = Integer.parseInt(testProps.getProperty("timeoutConnect"));
        int timeoutReceive = Integer.parseInt(testProps.getProperty("timeoutReceive"));
		...
		if ("true".equalsIgnoreCase(uddiEnabled)) {
            mediatorClient = new MediatorClient(uddiURL, wsName, timeoutConnect, timeoutReceive);
			for (int i = 0; i < NR_SUPPLIERS; i++){
				supplierClients[i] = new SupplierClient(uddiURL, supplierNames[i]);
            }
		} else {
            mediatorClient = new MediatorClient(wsURL, timeoutConnect, timeoutReceive);
			for (int i = 0; i < NR_SUPPLIERS; i++){
