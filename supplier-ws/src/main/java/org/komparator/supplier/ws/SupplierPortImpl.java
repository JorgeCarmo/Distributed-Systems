...
public class SupplierPortImpl
	implements SupplierPortType {
	...
	public String buyProduct(String productId, int quantity)
		...
		//check productId
		if(productId == null){
			throwBadProductId("Product description cannot be null!");
		}
		productId = productId.trim();
		if (productId.length() == 0){
			throwBadProductId("Product identifier cannot be empty or whitespace!");
		...
		//check quantity
		if(quantity <= 0){
			throwBadQuantity("Quantity cannot be less than 1!");
		}
		String purchaseID = "";
		Supplier supplier = Supplier.getInstance();
		try {
			...
			throwInsufficientQuantity("Product quantity is insuficient!");
		}
